...
public class UserService {
    ...
    public List<User> userList(UserSearchCriteria criteria) {
        return userMap.values().stream()
                ...
                .filter(user -> user.getUserRole() == criteria.getRole())
                .filter(user -> criteria.getSearchType().getKeyExtractor().apply(user).contains(criteria.getQuery()))
                .filter(user -> user.getCreatedAt().isAfter(criteria.getStartAt()))
                .filter(user -> user.getCreatedAt().isBefore(criteria.getEndAt()))
